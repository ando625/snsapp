...
これまで学んできた仕組みをさらに進化させて、Laravelの知識に加え、フロントエンドのモダンな技術である**Next.js (App Router)**を初めて導入。バックエンドと完全に切り離した構成、**「スマホアプリみたいに画面がスイスイ動く心地よさ」**と、**「書いた内容に合わせてAIが声をかけてくれる優しさ」**を形にしたいと思って挑戦しました。
...
2. **AIが「気持ち」に反応する演出**
* 投稿した瞬間に、Pythonというプログラムが内容を読み取って、「点数」をつけてくれます。
* ただ点数を出すだけじゃなくて、「楽しそうだね！」「ゆっくり休んでね」とメッセージを出したり、枠の色を変えたりして、**「楽しさを共有してさらにワクワクできたり、ふとした優しい言葉で気持ちがふんわり和んだりする」ような、心に寄り添う工夫**をしました。
...
| **3. 内容を判別** | Python (AI) | 本文を分析し、0〜100点のスコアを算出 |
